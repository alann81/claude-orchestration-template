...
<output>
Create/modify files:
- `./services/telegram-bot/src/routing/effort_classifier.py`
- `./services/telegram-bot/src/models/effort.py`
</output>

<verification>
```bash
cd services/telegram-bot && pytest tests/test_effort_classifier.py -v
```
</verification>
...
Port from scripts/daily-brief.py to the telegram-bot architecture.
```
...
6. Deployment prompt if telegram-bot modified
...
Target: services/telegram-bot/src/
...
Task(
  subagent_type="Explore",
  prompt="""
  Find all email handlers in services/telegram-bot/src/.
...
Task(
  ...
  Use local Ollama model (qwen2.5-coder:32b).
...
**How to invoke:**
```
"@debug, tests keep failing with KeyError: 'user_id' in email_handler.py"
...
- files: services/telegram-bot/src/email_handler.py
- task_file: tasks/detail/feat-044.md
- test_command: pytest tests/test_email_handler.py -v
```
...
integration_targets:
- services/telegram-bot/src/routing/__init__.py
- services/telegram-bot/src/routing/router.py
...
"@code-sentinel, do a full security review of services/telegram-bot/src/auth/"
```
...
**What happens:**
- Uses qwen2.5-coder:32b on local GPU server
- Zero cloud API cost
...
Fixed: Added null check in email_handler.py
