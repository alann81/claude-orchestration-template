...
1. **Detect Production-Affecting Changes**
   ```bash
   git diff --name-only HEAD~1 | grep -E "^telegram-bot/"
   ```

2. **If telegram-bot/ modified**, ASK USER:
   > "This task modified telegram-bot/. Deploy to production production-bot (LXC <container-id>)?"
...
[Deployment] telegram-bot/ not modified - no deployment needed
...
[Deployment] telegram-bot/ modified - user declined deployment
...
[Deployment] telegram-bot/ modified - deployed to LXC <container-id>
