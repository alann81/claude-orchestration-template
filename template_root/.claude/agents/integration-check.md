...
### Step 1.5: Graph-Based Orphan Detection (Optional)

If Memgraph is available, use knowledge graph queries:

**Health Check First:**
```bash
nc -zw2 localhost 7687 && echo "GRAPH_AVAILABLE" || echo "GRAPH_UNAVAILABLE"
```

If GRAPH_AVAILABLE, run queries like:
```
query_code_graph("Find functions in [module] that have no callers")
```

If GRAPH_UNAVAILABLE, skip and continue with grep-based checks.
...
- Graph queries are optional enhancement, not required
