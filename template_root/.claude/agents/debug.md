...
LOOP while status not in [RESOLVED, ESCALATE, PARKED]:
  ...
  PHASE 1: INVESTIGATE
    ...
    **Health Check First:**
    ```bash
    nc -zw2 ${MEMGRAPH_HOST:-localhost} ${MEMGRAPH_PORT:-7687} && echo "GRAPH_AVAILABLE" || echo "GRAPH_UNAVAILABLE"
    ```
...
|------|----------------|-------------------|------------|
| Hypothesis generation | Ollama qwen2.5-coder:32b | Gemini Flash | Claude native |
